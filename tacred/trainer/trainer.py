...
from utils import soft_label_loss
...
class Trainer:
    ...
    def train(self, need_validation=False):
        print('start')
    ...
    def additional_train(self, optimizer, need_validation=False):
        self.start_epoch = 1
        self.optimizer = optimizer
    ...
    def test(self, class_labels=[0]+list(range(2, 42))):
        self._valid_epoch(0, class_labels=class_labels)
    ...
    def get_clean_level(self):
        ...
        """
        self._valid_epoch(0)
        MC = self.ref_labels_id
        attn_arr = np.abs(self.attn_score_ls_ls)
        ans_arr = self.ans_ls_ls
        mapping_dict = {j: i for i, j in enumerate(sorted(MC))}
        split_avg = []
        for target_id in MC:
            idx = np.where(ans_arr == target_id)[0] # idx of target class, MC 
            n_select = sum(np.argmax(attn_arr[idx], axis=1) == mapping_dict[target_id]) # number of aligned attention map
            _v = n_select/len(idx) # 1 - noise level: aligned instances of MC / total number of instances of MC
            split_avg.append(_v)
        return max(0.1, round(np.mean(split_avg), 3))
    ...
    def _train_epoch_exponential(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.data_loader, 1):
            ...
            try:
                ...
                non_calc_idx = torch.any(torch.softmax(out, dim=1)>=thr, dim=1).detach().cpu() # confidence exceed threshold # non cor
    ...
    def _valid_epoch(self, epoch, class_labels):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.val_loader, 1):
                try:
                    ...
                    out, _, _, attn_score, gating = self.model(input_ids=input_ids, attention_mask=input_mask, sub_idx=ss, obj_idx=os, attn_guide=None, device=self.config.device)
        ...
        gating_ls = np.concatenate(gating_ls)
        self.pred_raw = np.concatenate(pred_ls)
        ...
        if not self.is_test:
            report = classification_report(self.ans_ls_ls, self.pred_ls_ls)
        ...
        if self.is_test or self.is_augcheck or self.need_attn:
            with open(self.config.source + str(batch_idx) + '_' + str(running_loss)[-4:-1] + '_' +  self.logger.name + '_pred_gating.pickle', 'wb') as f_pk:
                pickle.dump(gating_ls, f_pk)   
                
            with open(self.config.source + str(batch_idx) + '_' + str(running_loss)[-4:-1] + '_' +  self.logger.name + '_pred_raw.pickle', 'wb') as f_pk:
                pickle.dump(self.pred_raw, f_pk)   
    ...
    def _get_aug_idx_withfactor(self, aug_num, selected_instance_id):
        ...
        for target in self.ref_labels_id: # MCs label num
            ...
            if len(sent_ids) < 1:
                continue
            # aug_cap[target] = 150//len(sent_ids)
            ...
            for j, s_id in enumerate(sent_ids):
                ...
                candidate_instance = self.aug_id_ls_ls[sent_idx] # 실제 instance에서 idx
    ...
    def _get_train_aug_idx_withfactor(self, target_num):
        ...
        # select label
        attn_abs = np.abs(self.attn_score_ls_ls)
        print(attn_abs.shape)
        selected_instance_idx = {}

        for target in self.ref_labels_id: # MCs label num
            selected_instance_idx[target] = {"selected": None}
            relative_trg_idx = self.ref_labels_id.index(target) # [12, ..] 에서 idx
    ...
    def _save_checkpoint(self, epoch, is_additional_train=False):
        ...
        if is_additional_train:
            ...
            filename = str(self.config.checkpoint_dir + '{}-addtional-checkpoint-epoch{}.pth'.format(self.logger.name, epoch))
            ...
            filename = str(self.config.checkpoint_dir + '{}-checkpoint-epoch{}.pth'.format(self.logger.name, epoch))
